...
void main() {
  ...
  if (uEnableMouse) {
    float yaw = (uMouse.x - 0.5) * uParallax * 0.45;
    float pitch = (uMouse.y - 0.5) * uParallax * 0.45;
    c = cos(yaw); s = sin(yaw);
    dir = mat3(c, 0.0, s, 0.0, 1.0, 0.0, -s, 0.0, c) * dir;
  ...
  vec3 body = mix(uWaveColor, uCrestColor, clamp(pos.z * 0.09 + 0.5, 0.0, 1.0));
  vec3 col = mix(uHorizonColor, body, t);
  col *= uBrightness;

  float alpha = clamp(t * 1.35, 0.0, 1.0) * uOpacity;
  if (uGrain > 0.5) {
...
const GradientWaves: React.FC<GradientWavesProps> = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("WebGL GradientWaves not supported, falling back gracefully:", err);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    if (hc) { hc[0] = h[0]; hc[1] = h[1]; hc[2] = h[2]; }
    if (wc) { wc[0] = w[0]; wc[1] = w[1]; wc[2] = w[2]; }
    if (cc) { cc[0] = cr[0]; cc[1] = cr[1]; cc[2] = cr[2]; }
  }, [
    horizonColor,
    waveColor,
    crestColor,
  ...
  return <div ref={containerRef} className={`absolute inset-0 overflow-hidden pointer-events-none ${className}`.trim()} />;
};
